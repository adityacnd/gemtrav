...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Sep 29, 2023 at 02:12 AM
...
CREATE TABLE `explore` (
  ...
  `isi` text NOT NULL,
  `kategori` enum('Berita','Tips','Blog') NOT NULL,
...
INSERT INTO `explore` (`id`, `judul`, `isi`, `kategori`, `tgl_publikasi`, `penulis`, `status`, `jumlah_like`) VALUES
(23, 'Tips Liburan Aseek', '<p>Musim liburan hampir tiba! Abis kerja capek-capek selama beberapa bulan, selalu berhadapan dengan polusi dan macetnya perkotaan,&nbsp;<em>nggak&nbsp;</em>ada salahnya&nbsp;<em>kok&nbsp;</em>kamu menyisihkan sedikit waktu untuk pergi liburan. Hidup&nbsp;<em>balance&nbsp;</em>itu penting,&nbsp;<em>lho</em>!</p><p>Faktanya, liburan bisa bantu kamu melepaskan rasa penatmu, dan membangkitkan semangatmu untuk kembali bekerja.</p><p>&nbsp;</p><p><em>Nggak&nbsp;</em>perlu sedih dan menahan-nahan diri untuk berangkat liburan, kamu bisa kok menyiasati hal tersebut dengan cara liburan&nbsp;<em>smart,&nbsp;</em>asik tapi tetap hemat. Ayo ikuti tips berikut ini:</p><p>1.&nbsp;Persiapkan Dana&nbsp;<em>Travelling</em></p><p>Agar tidak mengganggu&nbsp;<em>cash flow&nbsp;</em>kamu, ada baiknya persiapkan dana untuk&nbsp;<em>travelling</em>&nbsp;jauh-jauh hari sebelum waktu rencana keberangkatanmu, terutama bagi kamu yang memang memiliki hobi&nbsp;<em>tavelling</em>. Usahakan tidak mengeluarkan uang lebih dari anggaran yang telah kamu persiapkan sejak awal, sehingga tidak mengganggu anggaran lainnya. Kamu boleh menabung 3 bulan, 6 bulan sebelumnya, atau lebih. Berangkat liburan secara mendadak dengan memanfaatkan pendapatan kamu sebulan, hanya akan membuat kamu &lsquo;tekor&rsquo; alias akhir bulan hanya makan mie instan!&nbsp;Dan JANGAN GUNAKAN cicilan kartu kredit untuk membiayai liburanmu, pokoknya jangan!</p><p>2.&nbsp;Hindari Musim Liburan</p><p>Saat musim liburan atau&nbsp;<em>high season,&nbsp;</em>harga barang dan jasa akan naik pada hampir semua tempat wisata. Oleh karena itu, selain dapat menikmati tempat liburan dengan lebih nyaman karena lokasi cenderung lebih sepi pengunjung, ada baiknya kamu berangkat saat&nbsp;<em>low season&nbsp;</em>karena harga yang ditawarkan biasanya lebih murah.</p><p>3.&nbsp;Pesan Tiket Tranportasi dan Penginapan Lebih Awal</p><p>Pesan tiket transportasi dan penginapan sejak awal. Hindari memesan dalam waktu dekat, apalagi membeli tiket di tempat atau&nbsp;<em>go-show,&nbsp;</em>karena harga yang ditawarkan lebih mahal. Selain memesan lebih awal, kamu juga bisa mengunjungi pameran liburan/&nbsp;<em>travel fair&nbsp;</em>untuk mendapatkan penawaran harga yang lebih murah serta promo potongan harga yang menarik. Agar lebih hemat dan praktis, kamu juga bisa memilih paket liburan menarik yang ditawarkan oleh beberapa&nbsp;<em>travel agent</em>, biasanya sudah termasuk tiket pesawat, penginapan dan konsumsi.</p><p>4.&nbsp;Siapkan Kebutuhan Pokok Sebelum Keberangkatan</p><p>Obat-obatan, peralatan mandi, makanan ringan, dan kebutuhan yang sekiranya akan diperlukan selama berwisata, lebih baik dipersiapkan sebelum keberangkatan. Barang-barang tersebut biasanya dijual dengan harga yang lebih mahal di sekitar tempat wisata, mengingat hampir setiap orang pasti membutuhkannya.</p>', 'Berita', '2023-09-29', '', 'on', 0);

-- --------------------------------------------------------

--
-- Table structure for table `exploreesss`
--

CREATE TABLE `exploreesss` (
  `exploreees_id` int(11) NOT NULL,
  `judul` varchar(255) NOT NULL,
  `isi` text NOT NULL,
  `tgl_publikasi` date NOT NULL,
  `penulis` varchar(100) NOT NULL,
...
INSERT INTO `messages` (`message_id`, `sender_id`, `receiver_id`, `message_text`, `timestamp`, `status`) VALUES
(136, 8, 8, 'halo admin 2', '2023-09-26 06:08:24', 'proses'),
(137, 8, 7, 'tutor dek', '2023-09-28 01:25:33', 'proses'),
(141, 8, 10, 'halo ngabs', '2023-09-28 23:43:27', 'proses'),
(142, 8, 10, 'tahmid ganteng:D', '2023-09-28 23:47:31', 'proses'),
(148, 8, 10, 'gw ganteng bet cooyy', '2023-09-28 23:52:16', 'proses'),
(149, 8, 10, 'gw ganteng bet cooyy', '2023-09-28 23:54:14', 'proses');
...
(22, 'customer', 'dumy', 'dum@d', 'di bumi Allah yang luas ini', '121313131313123', '0192023a7bbd73250516f069df18b500', 'on'),
(23, 'customer', 'sssssssss', 'wokrowkorkwkw', 'dimana aja', 'admadni', '0192023a7bbd73250516f069df18b500', 'on'),
(24, 'superadmin', 'admin', 'admin@a.com', 'di bumi Allah', '081808011', '47bce5c74f589f4867dbd57e9ca9f808', 'on');
...
--
-- Indexes for table `exploreesss`
--
ALTER TABLE `exploreesss`
  ADD PRIMARY KEY (`exploreees_id`);
...
--
-- AUTO_INCREMENT for table `exploreesss`
--
ALTER TABLE `exploreesss`
  MODIFY `exploreees_id` int(11) NOT NULL AUTO_INCREMENT;
